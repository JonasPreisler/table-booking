class OrdersController < ApplicationController
  ...
  def update
    @order = Order.find(params[:id])
  	...
  	if @order.update(params.require(:order).permit(:customer_id, :staff_id, :delivery_date, :order_amount, :order_paid, :order_status, :order_details, :zip, :address, :city, :first_name, :last_name))
